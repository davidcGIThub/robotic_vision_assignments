...
MAX_RANGE = 950		    # Closed loop position mode can only send relative position +/- 1000
OPEN_LOOP_LIMIT = 0.7   # 0.7 # Open loop speed limit to fraction of full speed
...
# DIN3 and DIN4 are not used anymore 01/29/2022
X_MOTOR = 1		    # Channel 2 is used for X
Y_MOTOR	= 2		    # Channel 1 is used for Y
X_SWITCH = 3		# X switches are wired for horizontal
Y_SWITCH = 4		# Y switches are wired for vertical
...
# Catcher Settings
CATCHER_H = 5.0     #    # 16.0	# catcher height in inches
CATCHER_W = 5.0     #    # 20.0	# catcher width in inches
X_INCH_COUNTS = 85		# # of encoder counts per linear inch = 4 (quadrature) * encoder pulses per linear inch.  This depends on the gear ratio
Y_INCH_COUNTS = 218	    # # of encoder counts per linear inch = 4 (quadrature) * encoder pulses per linear inch.  This depends on the gear ratio
X_MOTOR_PPR = 50	    # Motor pulses per revolution.  Controller get 4 counts per pulse.
Y_MOTOR_PPR = 50        # Motor pulses per revolution.  Controller get 4 counts per pulse.

# Close Loop Parameters
Y_POS_VEL = 20      # 4000 Position Mode Velocity (RPM)
Y_TURN_MM = 1000    # Position Turns Min to Max
Y_PID_KP = 150		# 150 Proportional Gain, 0~250 and 150 means 15.0
Y_PID_KI = 0		# Integral Gain, 0~250 and 150 means 15.0
Y_PID_KD = 200		# 200 Differential Gain, 0~250 and 150 means 15.0
Y_PID_ICAP = 100	# Integrator Limit (%)
# Close Loop Parameters
X_POS_VEL = 20      # # 4000 Position Mode Velocity (RPM)
