...
class RoboteqWrapper:
    ...
    def __init__(self, Port):
        ...
        command = "^DINL {}\r".format(3, 1)  # 63 doesn't seem to amke any difference
        self.Device.send_command(command)  # This value will set din to rnu script
        command = "^DINL {}\r".format(4, 1)  # 63 doesn't seem to amke any difference
        self.Device.send_command(command)  # This value will set din to rnu script

        # ^DINA cc (aa + mm) cc:Channel number, aa = 1 : safety stop,  mm = mot1 * 16 + mot2 * 32
        command = "^DINA {} {}\r".format(X_SWITCH, X_OFFSET + 1)
        self.Device.send_command(command)  # This value will set din to rnu script

        # ^DINA cc (aa + mm) cc:Channel number, aa = 1 : safety stop,  mm = mot1 * 16 + mot2 * 32
        command = "^DINA {} {}\r".format(Y_SWITCH, Y_OFFSET + 1)
        self.Device.send_command(command)  # This value will set din to rnu script
    ...
    def SetToDefault(self):
        ...
        for motor in motors:
            ...
            self.Device.send_command(
                command)  # Aceleration is 0.1RPM per second.  1,000 will acelerate 100RPM per esecond
            ...
            command = "^CLERD {} {}\r".format(motor, 2)
            self.Device.send_command(
...
###################################### Movement functions
    ...
    def MoveAtSpeed(self, XSpeed, YSpeed):
        ...
        XSpeed = -MAX_RANGE if (XSpeed < -MAX_RANGE) else XSpeed
        XSpeed = MAX_RANGE if (XSpeed > MAX_RANGE) else XSpeed
        YSpeed = -MAX_RANGE if (YSpeed < -MAX_RANGE) else YSpeed
        YSpeed = MAX_RANGE if (YSpeed > MAX_RANGE) else YSpeed
    ...
    def MoveToXY(self, x, y):
        ...
        y_relative = MAX_RANGE if (y_relative >= MAX_RANGE) else y_relative
        y_relative = -MAX_RANGE if (y_relative <= -MAX_RANGE) else y_relative

        x_relative = x * 2000 / CATCHER_W # Total distance travel is normalized to -1000 ~ +1000
        x_relative = MAX_RANGE if (x_relative >= MAX_RANGE) else x_relative
        x_relative = -MAX_RANGE if (x_relative <= -MAX_RANGE) else x_relative
        Speed = [0]*3       # Need 3 but the first one (0) is not used
